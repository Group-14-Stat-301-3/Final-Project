...
member_preds <- test_mlb %>%
  select(ops) %>%
  bind_cols(predict(mlb_model_stacked_fitted, test_mlb, members = TRUE))

# store results
results_ensemble <- map_dfr(member_preds, rmse, truth = ops, data = member_preds) %>%
...
From the testing data, a final RMSE score of 6.055356 was obtained, outperforming all of the other member models that comprises it. As this score is very low, the evidence supports the efficacy of the ensemble model as a strong, reliable predictor of the on-base-plus-slugging percentage in baseball.
